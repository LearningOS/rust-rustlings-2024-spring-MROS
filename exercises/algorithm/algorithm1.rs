            ...
            loop {
                match (head_a, head_b) {
                    (Some(ptr_a), Some(ptr_b)) => {
                        let val_a = (std::ptr::read(ptr_a.as_ptr())).val;
                        let val_b = (std::ptr::read(ptr_b.as_ptr())).val;
                        if val_a < val_b {
                            ret.add(val_a);
                    ...
                    (Some(ptr_a), None) => {
                        ret.add((std::ptr::read(ptr_a.as_ptr())).val);
                        head_a = (*ptr_a.as_ptr()).next;
                    }
                    (None, Some(ptr_b)) => {
                        ret.add((std::ptr::read(ptr_b.as_ptr())).val);
                        head_b = (*ptr_b.as_ptr()).next;
                    }
                    (None, None) => {
                        break;
